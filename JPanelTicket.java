...
//  EAN-13
//  In-store variable barcode
                } else if (((sCode.length() == 13) 
                        && (sCode.startsWith("2"))) 
                        || ((sCode.length() == 12) 
                        && (sCode.startsWith("2")))) {                   
                try {
                    ProductInfoExt oProduct = dlSales.getProductInfoByShortCode(sCode);
...
//  PRICE - SET value decimals
                        switch (sVariableTypePrefix) {
                            case "02":
                                dUnits = (Double.parseDouble(sVariableNum) / 100) / oProduct.getPriceSell(); 
                                break;
                            case "20":
                                dUnits = (Double.parseDouble(sVariableNum) / 100) / oProduct.getPriceSell(); 
                                break;
                            case "21":
                                dUnits = (Double.parseDouble(sVariableNum) / 10) / oProduct.getPriceSell();                                
                                break;
                            case "22":
...
//  WEIGHT - SET value decimals                                 
                            case "23":
                                weight = Double.parseDouble(sVariableNum) / 1000; //Weight in kg with three decimals (e.g. 1,234 kg)
                                dUnits = weight;
                                break;
                            case "24":
                                weight = Double.parseDouble(sVariableNum) / 100; //Weight in kg with two decimals (e.g. 12,34 kg)
                                dUnits = weight;                                                             
                                break;
                            case "25":
                                weight = Double.parseDouble(sVariableNum) / 10; //Weight in kg with one decimals (e.g. 123,4 kg)
                                dUnits = weight;                                                            
                                break;
                            default:
                                break;
...
//  PRICE - Assign var's
                            case "02":
                                ...
                                dUnits = (Double.parseDouble(sVariableNum) / 100) / oProduct.getPriceSellTax(tax);
                            ...
                            case "21":
                                dPriceSell = oProduct.getPriceSellTax(tax) / (1.0 + tax.getRate());
                                dUnits = (Double.parseDouble(sVariableNum) / 10) / oProduct.getPriceSellTax(tax);
                                oProduct.setProperty("product.price", Double.toString(oProduct.getPriceSell()));                                    
                                break;                                
                            case "22":
                                dPriceSell = oProduct.getPriceSellTax(tax) / (1.0 + tax.getRate());
                                dUnits = (Double.parseDouble(sVariableNum) / 1) / oProduct.getPriceSellTax(tax);
                                oProduct.setProperty("product.price", Double.toString(oProduct.getPriceSell()));                                    
                                break;
// WEIGHT - Assign var's
                            case "23":
                                weight = Double.parseDouble(sVariableNum) / 1000;   //3 decimals (e.g. 1.234 kg)
                                dUnits = weight;
                                oProduct.setProperty("product.weight", Double.toString(weight));
                                oProduct.setProperty("product.price", Double.toString(dPriceSell));                                                                    
                                break;                                
                            case "24":
                                weight = Double.parseDouble(sVariableNum) / 100;    //2 decimals (e.g. 12.34 kg)
                                dUnits = weight;
                                oProduct.setProperty("product.weight", Double.toString(weight));
                                oProduct.setProperty("product.price", Double.toString(dPriceSell));                                                                    
                                break;                                
                            case "25":
                                weight = Double.parseDouble(sVariableNum) / 10;     //1 decimal (e.g. 123.4 kg)
                                dUnits = weight;
                                oProduct.setProperty("product.weight", Double.toString(weight));
                                oProduct.setProperty("product.price", Double.toString(dPriceSell));                                    
                                break;
...
// I            PPPPP   S       CCCC    K                        
// 1            23456   7       891011  12 Human
// 0            12345   6       78910   11 Machine
...
*/
                ...
                try {
                    ProductInfoExt oProduct = dlSales.getProductInfoByUShortCode(sCode);
                    ...
                    if(oProduct == null) {
                        ...
                        String sVariableNum = sCode.substring(7, 11);           // grab the price from the code
                        ...
                        TaxInfo tax = taxeslogic.getTaxInfo(                    // get the prod tax rate
                                oProduct.getTaxCategoryID()                     // check Customer's tax 
                                , m_oTicket.getCustomer());

                        oProduct.setProperty("product.price"                    // set product SellPrice property
...
// END OF BARCODE            
            ...
            if ((cTrans == '\u0008') || (cTrans == '\u0008')) {                             // BACKSPACE Eliminar Linea
           ...
           if ((cTrans == '\u0045') || (cTrans == '\u0065')) {                          //F Editar Productos
           ...
           if ((cTrans == '\u003C')) {                                                  // Flecha Abajo panel lista
                stateToZero();
                m_ticketlines.selectionDown();                
            } 
            ...
            if (cTrans == '\u007f') { 
                ...
                m_jEditLine.doClick();
                    ...
                    && m_App.getAppUserView().getUser().hasPermission("sales.EditLines")) {
                ProductInfoExt product = getInputProduct();
                addTicketLine(product, 1.0, -product.getStockVolume());
                ...
                addTicketLine(product, getPorValue(), product.getPriceSell());
                    ...
                    && m_App.getAppUserView().getUser().hasPermission("sales.EditLines")) {
                ProductInfoExt product = getInputProduct();
                addTicketLine(product, getPorValue(), -product.getStockVolume());
...
/** This method is called from within the constructor to
    ...
    private void initComponents() {
        ...
        m_jPanelCentral.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                m_jPanelCentralMouseClicked(evt);
            }
        });
    ...
    private void jbtnMooringActionPerformed(java.awt.event.ActionEvent evt) {                                            
// Display vessel selection box on screen if reply is good add to the ticket
    ...
    private void j_btnRemotePrtActionPerformed(java.awt.event.ActionEvent evt) {                                               
    ...
    private void m_jKeyFactoryActionPerformed(java.awt.event.ActionEvent evt) {                                              
        // TODO add your handling code here:
    }                                             

    private void m_jTaxActionPerformed(java.awt.event.ActionEvent evt) {                                       
        // TODO add your handling code here:
    }                                      

    private void btnReprint1ActionPerformed(java.awt.event.ActionEvent evt) {                                            
    ...
    private void webCBCustomerActionPerformed(java.awt.event.ActionEvent evt) {                                              
        if (listener  != null) { 
            listener.stop(); 
        }
    ...
    private void jCheckStockActionPerformed(java.awt.event.ActionEvent evt) {                                            
    ...
    private void jCheckStockMouseClicked(java.awt.event.MouseEvent evt) {                                         
    ...
    private void m_jaddtaxActionPerformed(java.awt.event.ActionEvent evt) {                                          
        m_jKeyFactory.requestFocus();
    }                                         

    private void webCBCustomerKeyReleased(java.awt.event.KeyEvent evt) {                                          
            m_jKeyFactory.requestFocus();
    }                                         

    private void webCBCustomerMouseReleased(java.awt.event.MouseEvent evt) {                                            
            m_jKeyFactory.requestFocus();
    }                                           

    private void m_jPanelCentralMouseClicked(java.awt.event.MouseEvent evt) {                                             
      // TODO add your handling code here:
    }                                            
